...
const allItems = [...allDevelopments, ...allUpgrades].sort((a, b) => a.name.localeCompare(b.name))
    ...
    // Keeps all stores that are purchased
        const purchased = allItems.filter(development => purchasedDevelopments.some(p => p.name === development.name))
    
        // Keeps all stores that are NOT purchased
        const available = allItems.filter(development => !purchasedDevelopments.some(p => p.name === development.name))
